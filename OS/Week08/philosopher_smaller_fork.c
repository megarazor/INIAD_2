...
void* philosopher_thread(void* arg)
{
    ...
    for (i = 0; i < DAYS; i++) {
        ...
        pthread_mutex_lock(&fork_mutex[smaller]);
        printf("%s picked up the smaller fork\n", names[phil]);
        pthread_mutex_lock(&fork_mutex[bigger]);
        printf("%s picked up the bigger fork\n", names[phil]);

        eat(phil, smaller, bigger);

        printf("%s returned the bigger fork\n", names[phil]);
        pthread_mutex_unlock(&fork_mutex[bigger]);
        printf("%s returned the smaller fork\n", names[phil]);
        pthread_mutex_unlock(&fork_mutex[smaller]);
    }
